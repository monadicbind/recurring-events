...
instance RepeatScheduleHelper EveryNTimes where
  ...
  scheduleLogic (EveryXDays days initDay) compareDay =
    let initInternalDay = getMonthOfDateWithWeekAndOrdinal initDay
        compareToInternalDay = getMonthOfDateWithWeekAndOrdinal compareDay
    in day initInternalDay `mod` days == day compareToInternalDay `mod` days
